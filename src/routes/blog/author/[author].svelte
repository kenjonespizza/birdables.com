...
<section class="bg-gray-blue">
  <div class="max-w-7xl mx-auto pt-10 pb-12 px-4 sm:px-6 md:px-8">
    <Breadcrumb current={author.pageInfo.name} steps={[{title:"Blog", link:"/blog"},{title:"Authors", link:"/blog/authors"}]} />
    <div class="md:flex md:justify-between items-end">
      <div class="flex space-x-10 pt-8">
        <img loading=lazy class="w-96 drop-shadow-card rounded-lg" src={urlFor(author.image).quality(80).size(400, 500)} alt={author.pageInfo.name}>
       
        <div>
          <h2 class="text-4xl font-extrabold text-gray-900 sm:text-5xl sm:tracking-tighter lg:text-6xl">{author.pageInfo.name} <span class="tracking-tight italic text-gray-500 text-4xl">{author.position}</span></h2>
